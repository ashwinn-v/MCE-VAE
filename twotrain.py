...
def calc_loss(model, x, x_init, beta=1., n_sampel=4):
    ...
    if model.rec_loss == 'mse':
        ...
        #RE_tilde = -torch.sum((x*torch.log(x_rec) + (1-x)*torch.log(1-x_rec)))
    ...
    # likelihood_tilde = - (RE_tilde + RE_INV)/x.shape[0]
    ...
    loss_tilde = -likelihood + divergence_c + divergence_var
    loss = - likelihood + beta * divergence_var_tau + divergence_c
    losses = [loss_tilde, loss]

    return loss_tilde, RE/x.shape[0], divergence_var_tau, divergence_c
