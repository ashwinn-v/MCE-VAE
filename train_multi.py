...
def train_epoch(data, model, optim, epoch, num_epochs, N, beta):
    ...
    for (x, x_init) in data:
        ...
        optim.zero_grad()
        loss, reco_loss, divergence_var_tau, divergence_c,x_rec = calc_loss(model, x, x_init, beta = beta)


        losses = [reco_loss,divergence_var_tau, divergence_c]
        loss_data = {}
        grads = {}
        scale = {}
        mask = None
        ...
        #if not rep_variable.grad:
        for i in range(len(losses)):
            
            loss_data[i] = losses[i]
            losses[i].backward(retain_graph=True)
            grads[i] = []


            # if not rep_variable.grad:
            #     rep_variable_zeros = torch.zeros_like(rep_variable, requires_grad=True)
            #     grads[i].append(Variable(rep_variable_zeros, requires_grad=False))
            #     continue
            
            grads[i].append(Variable(rep_variable.grad.data.clone(), requires_grad=False))
            rep_variable.grad.data.zero_()

            print(rep_variable.shape,grads[i][0].shape)
        ...
        pdb.set_trace()
        sol, min_norm = MinNormSolver.find_min_norm_element([grads[i] for i in range(len(losses))])
        for i in range(len(losses)):
            scale[i] = float(sol[i])
        ...
        optim.zero_grad()
        opt = model(x, k)
        losses = ["loss1","loss2","loss3"]  ## we will get from loss function
        for i in range(len(losses)):
            if i > 0:
                loss = loss + scale[i]*losses[i]
            else:
                loss = scale[i]*losses[i]
        loss.backward()
        optim.step()
        # loss.backward()
        # optim.step()
